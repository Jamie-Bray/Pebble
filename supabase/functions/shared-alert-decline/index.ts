...
const redirectTargets = {
  declined: 'https://pebbleroutines.com/shared-alert/declined',
...
serve(async (req) => {
  ...
  return redirectTo(redirectTargets.declined);
});
...
async function findInvite(serviceClient: ReturnType<typeof createClient>, token: string) {
  const { data, error } = await serviceClient
    .from('shared_alert_invites')
    .select('id, contact_id, expires_at, declined_at, blocked_at')
